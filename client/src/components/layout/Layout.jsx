...
export default function Layout() {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-white">
            {/* Sidebar */}
            <aside className="w-64 bg-white border-r border-slate-100 flex flex-col flex-shrink-0 shadow-sm z-20">
                {/* Logo */}
                <div className="h-16 flex items-center px-6 border-b border-slate-100">
                    <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center mr-3">
                        <Zap className="w-4 h-4 text-white" />
                    </div>
                    <span className="text-lg font-bold text-slate-900 tracking-tight">Provexa</span>
                </div>
                ...
                <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                    {navItems.map((item) => (
                        ...
                        >
                            {({ isActive }) => (
                                <>
                                    <item.icon className={`w-4.5 h-4.5 mr-3 flex-shrink-0 transition-colors ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-600'}`} style={{width:'18px', height:'18px'}} />
                                    {item.label}
                                </>
                            )}
                        </NavLink>
                ...
                <div className="p-3 border-t border-slate-100">
                    <button
                        ...
                        className="flex w-full items-center px-3 py-2.5 text-sm text-slate-600 hover:bg-red-50 hover:text-red-600 rounded-xl font-medium transition-all group"
                    >
                        <LogOut className="w-4.5 h-4.5 mr-3 text-slate-400 group-hover:text-red-500" style={{width:'18px', height:'18px'}} />
                        Sign Out
                    </button>
                </div>
            </aside>
