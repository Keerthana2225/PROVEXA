                    ...
                    <div className="flex gap-2 ml-auto shrink-0">
                        ...
                        {!isHistory && (
                            <button
                                ...
                                className="flex items-center bg-primary hover:bg-primary/90 text-white px-5 py-2.5 rounded-xl font-semibold transition-colors shadow-sm shadow-primary/20 text-sm"
                            >
                                <Plus className="w-5 h-5 mr-1.5" /> Issue Item
                            </button>
                        )}
                    </div>
                </div>
                            ...
                            {isLoading ? (
                                [...Array(5)].map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        {[...Array(7)].map((_, j) => (
                                            <td key={j} className="px-6 py-4"><div className="h-4 bg-slate-100 dark:bg-slate-800 rounded w-24"></div></td>
                                        ))}
                                    </tr>
                                ))
                            ) : filtered.length === 0 ? (
                                        ...
                                        <tr key={issueId}
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-xs shrink-0">
                                                        {issue.employee?.name?.charAt(0).toUpperCase() || '?'}
                                                    </div>
                                                    <div>
                                                        <div className="font-bold text-slate-900 dark:text-white text-sm">{issue.employee?.name || 'Unknown'}</div>
                                                        <div className="text-[10px] text-slate-400 font-mono">{issue.employee?.emp_code || 'N/A'} · {issue.employee?.department || 'N/A'}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            {/* Item */}
                                            <td className="px-6 py-4">
                                                <div className="font-semibold text-slate-900 dark:text-slate-200 text-sm">{issue.item?.name || 'Unknown Item'}</div>
                                                <div className="text-[10px] text-slate-400 uppercase font-semibold tracking-wide">{issue.item?.category?.name || 'N/A'}</div>
                                            </td>

                                            {/* Qty */}
                                            <td className="px-6 py-4 text-center">
                                                <span className="inline-block font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg text-sm">{issue.quantity}</span>
                                            </td>

                                            {/* Date */}
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-medium text-slate-700 dark:text-slate-300">{dayjs(issue.issued_date).format('DD MMM YYYY')}</div>
                                                <div className="text-[10px] text-slate-400">Due: {dayjs(issue.next_due_date).format('DD MMM YYYY')}</div>
                                            </td>
